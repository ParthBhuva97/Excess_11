...
class AlarmBroadcastReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        if(intent!=null && context!=null){
            ...
            Log.i(ContentValues.TAG, "onReceive: Note:$note")
            notificationDialog(
                context,
                NotesViewActivity::class.java,
                note.title,
                note.Description,
                note
            )
    ...
    private fun notificationDialog(context: Context,cls:Class<*>,title:String,desc:String,note:Note){
        val notificationManager = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
        val NOTIFICATION_CHANNEL_ID = "parthbhuva"
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.O){
            val notificationChannel = NotificationChannel(NOTIFICATION_CHANNEL_ID,"Note Application",NotificationManager.IMPORTANCE_HIGH)
        ...
        val pendingIntent = PendingIntent.getActivity(context,Note.REMINDER_REQUEST_CODE,notificationIntent,PendingIntent.FLAG_UPDATE_CURRENT)
        val notificationBuilder = NotificationCompat.Builder(context, NOTIFICATION_CHANNEL_ID)
        notificationBuilder.setAutoCancel(true)
            .setDefaults(Notification.DEFAULT_ALL)
            .setWhen(System.currentTimeMillis())
            .setSmallIcon(R.mipmap.ic_launcher)
            .setTicker("NoteApplication-ParthBhuva")
            .setContentTitle(title)
            .setContentText(desc)
            .setContentInfo(desc + "Information")
            .setAutoCancel(true)
